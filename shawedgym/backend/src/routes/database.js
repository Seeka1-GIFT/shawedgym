...
// POST /api/database/reset-admin-password → Reset admin password to admin123
router.post('/reset-admin-password', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE users SET password = $1 WHERE email = $2 RETURNING id, email, first_name, last_name, role',
      [hash, adminEmail]
    );
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ 
        ...
        message: 'Admin user with email admin@shawedgym.com not found' 
      });
    }
