...
const getTrainers = async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM trainers ORDER BY first_name ASC');
    res.json({
      success: true,
...
const createTrainer = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO trainers (first_name, last_name, email, phone, specialization, experience, hourly_rate, status, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW()) RETURNING *',
      [first_name, last_name, email, phone, specialization, experience, hourly_rate, 'active']
    );
...
const updateTrainer = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE trainers SET first_name = $1, last_name = $2, email = $3, phone = $4, specialization = $5, experience = $6, hourly_rate = $7, status = $8, updated_at = NOW() WHERE id = $9 RETURNING *',
      [first_name, last_name, email, phone, specialization, experience, hourly_rate, status, id]
    );
