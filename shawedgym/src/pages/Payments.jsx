  ...
  const submitAddPayment = async (e) => {
    ...
    try {
      if (!newPayment.memberId || !newPayment.amount || !newPayment.method) {
        showError('Please fill in all required fields (Member, Amount, and Payment Method)');
        return;
      }

      const payload = {
        memberId: Number(newPayment.memberId),
        amount: Number(newPayment.amount),
        method: newPayment.method,
        description: newPayment.description || '',
        planId: newPayment.planId ? Number(newPayment.planId) : null
      };
      ...
      showError(err.message || 'Failed to add payment');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 p-6">
      ...
      <div className="mb-8">
        ...
        {isAdmin && (
          <div className="mt-4">
            <button onClick={openAddPaymentModal} className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors">Add New Payment</button>
          </div>
        )}
      </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            {isAdmin && (
              <button 
                ...
                className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105"
              >
                  ...
                  >
                    ...
                    {memberOptions.map(member => (
                      <option key={member.id} value={member.id}>
                        {member.first_name ? `${member.first_name} ${member.last_name || ''}`.trim() : (member.name || `Member ${member.id}`)}
                      </option>
                    ))}
                  </select>
                </div>
