...
const Members = () => {
  ...
  const handleDeleteMember = async (memberId) => {
    if (!(isAdmin || isCashier)) {
      showError('Only administrators or cashiers can delete members');
      return;
    }
    try {
  ...
  const handleUpdateMember = async (e) => {
    e.preventDefault();
    if (!(isAdmin || isCashier)) {
      showError('Only administrators or cashiers can update members');
      return;
    }
    try {
      ...
      if (newPhotoUrl && newPhotoUrl.startsWith('data:image')) {
        try {
          const resJson = await apiService.uploadBase64Image(newPhotoUrl);
      ...
      if (!newPhotoUrl && canvasRefEdit.current && videoRefEdit.current && videoRefEdit.current.videoWidth) {
        ...
        try {
          const resJson = await apiService.uploadBase64Image(base64);
          if (resJson?.success && resJson?.data?.url) newPhotoUrl = resJson.data.url;
        } catch (err) {
          console.error('Photo upload failed:', err);
        }
